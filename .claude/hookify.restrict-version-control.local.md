...
- `git add`, `commit`, `push`, `pull`, `fetch`, `merge`, `rebase`, `reset`, `checkout`, `switch`, `stash`, `cherry-pick`, `revert`, `tag`, `clean`, `clone`, `init`, `mv`, `rm`, `restore`, destructive `branch` operations
...
- `git submodule` (add, update, init, deinit, sync, status, foreach)
